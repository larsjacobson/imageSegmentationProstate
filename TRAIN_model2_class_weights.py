...
if __name__ == "__main__":
    ...
    y_train_labels = np.argmax(y_train, axis=-1)  # y_train is the training mask data

    # Calculate class weights
    weights = class_weight.compute_class_weight(
        class_weight='balanced',
        classes=np.unique(y_train_labels),
        y=y_train_labels.flatten()
    )

    # Create a dictionary to pass to class_weight in model.fit
    ...
    history = model.fit(
        ...
        class_weight=class_weights,  # Use the computed class weights here
        callbacks=[progbar_logger]
    )
    print("Training complete.")

    # Save the model
    model.save('prostate_segmentation_model_class_weights.h5')
    print("Model saved with class weights. Script finished.")